...
(function (window) {
  ...
  Record.prototype = {
    Init: function (ele, options) {
      ...
      // that.recordRTC = {}
      this.ele = ele
      window[ele + 'video'] = document.querySelector(ele)
  ...
  Record.prototype.successCallback = function (stream) {
    window[that.ele + 'video'].src = window.URL.createObjectURL(stream)

    window[that.ele + 'recordRTC'] = window.RecordRTC(stream, this.options)

    window[that.ele + 'recordRTC'].startRecording()
  }
   ...
   *
   */
  Record.prototype.stop = function (ele, cb) {
    console.log('that', window)
    window[ele + 'recordRTC'].stopRecording(function (audioVideoWebMURL) {
      window[ele + 'video'].pause()
      // console.log('that.recordRTC', that.ele.recordRTC)
      cb(window[ele + 'recordRTC'])
    })
    return this
  }
